  ...
  void run() {
    ...
    while(1) {
      ...
      if (packet_in.PopNB(packet_reg)) {
        if(packet_reg.dst == packet_reg.SA) {
          if(packet_reg.d_type == 0) { // weights
            ...
            if(DEBUG) cout << "SA " << id << " Pod " << packet_reg.x << " receive weight from SB " << packet_reg.src << "\n";
            weight = packet_reg;

          } else if(packet_reg.d_type == 1 && is_act_in == 0) { // activation
            
            if(DEBUG) cout << "SA " << id << " Pod " << packet_reg.x << " receive activation from SB " << packet_reg.src << "\n";
      ...
      if(is_act_in) { // do matmul and send result
        ...
        packet_reg.dst = packet_reg.AB;
        packet_reg.d_type = 2; // result type  

        if(DEBUG) cout << "SA " << id << " Pod " << packet_reg.x << " sending result to AB\n";
